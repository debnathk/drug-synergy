...
if __name__ == "__main__":
    ...
    exp_name = "synergy_concat_std" if args.standardize else "synergy_concat"
    run_dir, logger = setup_logging(experiment_name=exp_name)
    
    logger.info("="*80)
    logger.info("Starting Drug Synergy Prediction Training (End-to-End with Concatenation)")
    logger.info("="*80)
    ...
    # Model setup
    logger.info("Initializing SynergyModel (End-to-End with Concatenation)...")
    model = SynergyModel(
        mrna_dim=mrna_dim,
        mirna_dim=mirna_dim,
        prot_dim=prot_dim
